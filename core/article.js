...
async function abootstrap() {
    ...
    document.body.className = "b"+articles[id].class;
    ...
    for(var i=0;i<content.length;i++){
        if(content[i].type=="link"){
            if(content[i].class=="internal"){
                b=b+"<a id=c"+articles[id].class+" href=article.html?id="+content[i].id+"&lang="+actuallang+" class='internal'>"+content[i]["name-"+actuallang]+"</a>";
            }
        }
